...
import { duaCategories, getTajweedCategories, getLetters } from "@/Bottom/API/Aid";
import vocabularyData from "@/Bottom/Data/Aid/Arabic/Vocabulary.json";
...
export async function searchVerses(query: string): Promise<VerseResult[]> {
  ...
  for (const surahId of AVAILABLE_SURAHS_FOR_VERSE_SEARCH) {
    try {
      ...
      const { surahList: sl } = await import("@/Bottom/API/Quran");
