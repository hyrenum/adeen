...
import { Filter } from "Client/Component/Quran/Filter";
import {
  TrendingUp,
...
import { Progress_Ring } from "Client/Component/Quran/Goal/Progress";
...
const Quran = () => {
  ...
  const continueReadingUrl = continueReadingSurah
    ? `/Quran/Surah/${continueReadingSurah.id}?verse=${
        progress?.last_ayah_id || 1
      }`
    : "/Quran/Surah/1";
  ...
  const getGoalDisplay = () => {
    ...
    if (activeGoal.goal_type === "time_based") {
      ...
      const percentage = Math.min(
        100,
        (totalTodaySeconds / targetSeconds) * 100
      );
  ...
  const getView = () => {
    ...
    // Surah list (sorted by selected order)
    const sortedSurahs = (() => {
  ...
  return (
    <Layout>
          ...
          <Card className="p-4 sm:p-5 w-full group h-full">
            <div className="flex items-center justify-between mb-3">
              ...
              <Button size="sm" className="text-xs">
                {progress?.last_ayah_id
                  ? `Ayah ${progress.last_ayah_id}`
                  : "Start"}
              </Button>
            </div>
                        ...
                        <>
                          ...
                          <div className="flex items-center gap-2 text-xs">
                            ...
                            <span className="font-semibold text-foreground [.high-contrast_&]:group-hover:text-white [.high-contrast_&]:dark:group-hover:text-black">
                              {goalDisplay.remainingValue}{" "}
                              {goalDisplay.remainingLabel}
                            </span>
                          </div>
                        </>
                      )}
      ...
      <div className="flex justify-end mb-6 relative">
        <Button
          ...
          className={`flex items-center gap-2 ${
            filterType || selectedSurah ? "active" : ""
          }`}
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
        {/* Kalimah View */}
        {view.type === "kalimah" &&
          view.data.map((word, idx) => (
            <div key={idx}>
              <Link
                to={`/Quran/Surah/${selectedSurah}/Ayah/${selectedAyah}/Kalima/${
                  idx + 1
                }`}
        ...
        {/* Ayahs View */}
        {view.type === "ayahs" &&
          view.data.map((ayahNum) => (
            <div key={ayahNum}>
              <Button
                onClick={() =>
                  navigate(`/Quran/Surah/${selectedSurah}/Ayah/${ayahNum}`)
                }
