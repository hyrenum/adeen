// Client/Component/Aid/Navigator.tsx
...
import {
  ...
  DropdownMenuItem,
  DropdownMenuTrigger,
} from "Client/Component/UI/Dropdown-Menu";
...
function parseRoute(pathname: string) {
  ...
  if (parts[0] === "Hadith") {
    return {
      ...
      chapterId: parts[2],
      hadithId: parts[3] ? parts[3] : undefined, // Maintained as string string identifier support
    };
  }
  return null;
}
...
export function Aid_Navigator() {
  ...
  // 🌟 Connect component directly to the shared React Query cache line
  ...
  // ----- derived data structures managed cleanly from query cache -----
  ...
  const chaptersForDrill = useMemo(() => {
    ...
    const targetColl = corpus.collections.find(
      (c: any) => c.slug.toLowerCase() === drillCollection.toLowerCase()
    );
        ...
        <div className="overflow-y-auto space-y-0.5 max-h-full">
          {filteredCollections.map((c: any) => (
            <button
              ...
              className={cn(
                itemBase,
                (route?.mode === "hadith" && route.collectionSlug?.toLowerCase() === c.slug.toLowerCase()) &&
                  itemActive
              )}
            >
              {c.name}
  ...
  const buttonLabel = useMemo(() => {
    ...
    if (route?.mode === "hadith") {
      ...
      if (route.chapterId) {
        const targetColl = corpus?.collections?.find(
          (c: any) => c.slug.toLowerCase() === route.collectionSlug?.toLowerCase()
        );
