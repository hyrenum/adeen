...
import { Bismillah } from "Client/Component/Quran/Bismillah";
...
export const PageLines = memo(function PageLines({
  ...
  const buildWordClassName = (
    ...
    if (isVerseHighlighted && !isVerseMarker) {
      cls += "text-[hsl(var(--quran-hover))]";
    } else if (isActive) {
      cls += "text-foreground animate-pulse";
    } else if (isPlayingAudio) {
      cls += "text-[hsl(var(--quran-hover))] animate-pulse";
    } else if (isVerseEnd || isVerseMarker) {
      cls += "text-muted-foreground hover:text-[hsl(var(--quran-hover))] cursor-pointer";
    } else {
      if (!isUthmaniV4Font) {
        cls += "text-foreground hover:text-[hsl(var(--quran-hover))]";
      }
    }
    return cls;
  };
